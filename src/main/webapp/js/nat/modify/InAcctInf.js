...
//跳转到修改页面
function modify(index,rptdate){
	openSubTab(path+'/jsp/nat/modify/InAcctInfDetail.jsp?id='+index+'&rptdate='+rptdate,'个人借贷信息修改详细');
}
//条件查询
function doQuery(){
...
function confirm(seqNo){
	$.messager.confirm('确认','是否确认该笔贷款信息？',function(r){
		if(r){
			$.ajax({
				url : path + '/InAcctInf/confirm.html',
				type : "post",
				data : {
					// 主键
					"seqNo" : seqNo
