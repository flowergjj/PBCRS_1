...
function getentTable() {
	...
	$('#BalanceSheetTable').datagrid({
		striped : true,
		url : path + '/BalanceSheet/listPage.html',
		remoteSort : false,
		idField : 'BALANCESHEETSEQNO',
		...
		columns : [ [ {
			...
			formatter : function(value, row, index) {
				var str;
				var seqNo = row.BALANCESHEETBSSGMTSEQNO;
			...
			align : 'center',
			formatter : function(value, row, index) {
				var url = row.BALANCESHEETSEQNO.replace(/\\/g,
						"/");
...
/*				}else{
	...
	// 设置分页控件
	var p = $('#BalanceSheetTable').datagrid('getPager');
	$(p).pagination({
...
function modify(url,rptdate,mode){
	openSubTab('../jsp/ent/modify/BalanceSheetDetial.jsp?id='+url+"&mode="+mode+"&rptdate="+rptdate,'企业资产负债表信息修改详细');
}

//条件查询
function doQuery(){
	
	$('#BalanceSheetTable').datagrid('load',getQueryParam());
...
function getQueryParam() {
	...
	if(enbasinfseqno){
		queryObject.BALANCESHEETSEQNO=enbasinfseqno;
	}
...
function del(url){
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
		if(r){
			var param = {};
			param.BalanceSheetSeqNo = url;
			$.ajax({
				url : path + '/BalanceSheet/updateByIsDel.html',
				type : "post",
				data:param,
				async : false,
				success : function(data) {
