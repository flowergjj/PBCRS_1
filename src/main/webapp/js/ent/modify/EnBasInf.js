...
function getentTable() {
	...
	$('#entTable').datagrid({
		striped : true,
		url : path + '/EnBasInfBsSgmt/listPage.html',
		remoteSort : false,
		idField : 'BSSGMTSEQNO',
		...
		columns : [ [ {
			...
			formatter : function(value, row, index) {
				...
				var url = row.BSSGMTSEQNO.replace(/\\/g,
						"/");
...
*/					if(row.ISDEL == '1'){
	...
	// 设置分页控件
	var p = $('#entTable').datagrid('getPager');
	$(p).pagination({
...
function modify(url,rptdate,mod){
	openSubTab('../jsp/ent/modify/EnBasInfDetial.jsp?id='+url+'&mod='+mod+"&rptdate="+rptdate,'企业基本信息修改详细');
...
function doQuery(){
	...
	$('#entTable').datagrid('load',getQueryParam());
...
function del(url){
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
		if(r){
			var param = {};
			param.BsSgmtSeqNo = url;
			$.ajax({
				url : path + '/EnBasInfBsSgmt/updateByIsDel.html',
				type : "post",
				data:param,
				async : false,
				success : function(data) {
