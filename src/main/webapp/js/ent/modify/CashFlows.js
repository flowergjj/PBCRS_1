...
function getentTable() {
	...
	$('#CashFlowsTable').datagrid({
		striped : true,
		url : path + '/CashFlows/listPage.html',
		remoteSort : false,
		idField : 'CASHFLOWSSEQNO',
		...
		columns : [ [ {
			...
			formatter : function(value, row, index) {
				var str;
				var seqNo = row.CASHFLOWSBSSGMTSEQNO;
			...
			align : 'center',
			formatter : function(value, row, index) {
				var url = row.CASHFLOWSSEQNO.replace(/\\/g,
						"/");
...
/*				}else{
	...
	// 设置分页控件
	var p = $('#CashFlowsTable').datagrid('getPager');
	$(p).pagination({
...
function modify(url,rptdate,mode){
	openSubTab('../jsp/ent/modify/CashFlowsDetial.jsp?id='+url+"&mode="+mode+"&rptdate="+rptdate,'企业现金流量表修改详细');
}

//条件查询
function doQuery(){
	
	$('#CashFlowsTable').datagrid('load',getQueryParam());
...
function getQueryParam() {
	...
	if(enbasinfseqno){
		queryObject.CASHFLOWSSEQNO=enbasinfseqno;
	}
...
function del(url){
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
		if(r){
			var param = {};
			param.CashFlowsSeqNo = url;
			$.ajax({
				url : path + '/CashFlows/updateByIsDel.html',
				type : "post",
				data:param,
				async : false,
				success : function(data) {
