...
function showDetial(id){
	...
	if(id=="d1"){
		var type = "'310','340','350','410','420','430','440','610','620','630','640','650'";
		getRptType(type);
...
function loadEnReportModTable(){
	$('#EnReportModTable').datagrid({
		striped : true,
		url : path + '/reportMod/listPage.html',
		remoteSort : false,
		//idField : 'REPORTMODID',
...
function loadInReportModTable(){
	$('#MnMmbInfTable').datagrid({
		striped : true,
		url : path + '/reportMod/InlistPage.html',
		remoteSort : false,
		idField : 'REPORTMODID',
		...
		columns : [ [
			...
			formatter : function(value, row, index) {
					...
					if( value=='510') return '抵(质)押合同信息记录';
					if( value=='210') return '个人借贷账户记录';
					if( value=='220') return '个人授信协议信息记录 ';
